...
final class AnyEncodableTests: XCTestCase {
    ...
    func test_customEncoder를_주입했을때_인코더_설정대로_인코딩되어야_한다() throws {
        ...
        let encodable = AnyEncodable(MockEncodable(), jsonEncoder: cusomJSONEncoder)
        let value = try encodable.encode()
        XCTAssertEqual(#"{"key_case":"value"}"#, String(data: value, encoding: .utf8)!)
    }

    struct MockEncodable: Encodable {
        let keyCase: String = "value"
    }
}
